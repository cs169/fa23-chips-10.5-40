...
RSpec.describe CampaignFinance, type: :model do
  describe '.cycles' do
    it 'returns an array of years from 2010 to 2020' do
      expect(described_class.cycles).to eq((2010..2020).select(&:even?))
    end
  end
  ...
  describe '.find_from_top_twenty' do
    ...
    context 'when API call is successful' do
      ...
      it 'returns the data from the API' do
        results = described_class.find_from_top_twenty(search_params)
    ...
    context 'when API call fails' do
      ...
      it 'returns an empty array' do
        expect(described_class.find_from_top_twenty(search_params)).to eq([])
      end
    end
